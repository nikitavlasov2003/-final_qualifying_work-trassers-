...
<a href="https://cdwxg7lw833to68wt9cpbb.streamlit.app/" style="text-decoration:none; color:inherit; vertical-align:middle;">
...
Традиционные подходы к выбору скважин-кандидатов (в частности, под обработку СПС), как правило, основываются на инженерной интерпретации трассерных исследований, анализе фильтрационных откликов, уровня обводнённости, дебитов и других технологических показателей. Однако, такой анализ является трудоёмким, требует значительных временных и экспертных ресурсов, а также подвержен элементу субъективности.

### **Решение:**

Применение машинного обучения позволяет существенно повысить эффективность процесса подбора скважин-кандидатов за счёт автоматизации обработки признаков, учёта многомерных зависимостей между параметрами и обеспечения стандартизированного подхода на основе накопленных исторических данных. В данной работе реализована двухэтапная модель классификации на базе CatBoost с глубокой интерпретацией через SHAP (SHapley Additive exPlanations). Представленный далее подход является дополнением к инженерному анализу и позволяет ускорить принятие решений и минимизировать вероятность упущенных кандидатов. 
